...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${env.backendUrl}/api/v1/auth/register`, {
    ...
    if (typeof data.access_token !== "string" || !data.user) return NextResponse.json({ detail: "Registration could not be completed. Please try again." }, { status: 502 });
