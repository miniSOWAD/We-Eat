...
export function RegisterForm() {
  ...
  if (step === 1) return <form className="stack" onSubmit={requestOtp}>{error && <div className="error">{error}</div>}<div className="field"><label htmlFor="registerEmail">Email</label><input id="registerEmail" className="input" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required/><span className="help">A six-digit code will be delivered to this address. If no email arrives, the cloud email provider is not configured correctly.</span></div><button className="button buttonPrimary" disabled={busy}>{busy ? "Sending…" : "Send verification code"}</button></form>;
